...
def clustering(adata, n_clusters=7, radius=50, key='emb', method='mclust', start=0.1, end=3.0, increment=0.01,
    ...
    if method == 'mclust':
        ...
        adata.obs['stGRL'] = adata.obs['mclust']
    elif method == 'leiden':
        res = search_res(adata, n_clusters, use_rep='emb_pca', method=method, start=start, end=end, increment=increment)
        sc.tl.leiden(adata, random_state=0, resolution=res)
        adata.obs['stGRL'] = adata.obs['leiden']
    elif method == 'louvain':
        res = search_res(adata, n_clusters, use_rep='emb_pca', method=method, start=start, end=end, increment=increment)
        sc.tl.louvain(adata, random_state=0, resolution=res)
        adata.obs['stGRL'] = adata.obs['louvain']
    elif method =='k-means':
        adata = Kmeans_cluster(adata,num_cluster=n_clusters,used_obsm='emb_pca')
        adata.obs['stGRL'] = adata.obs['kmeans']

    if refinement:
        new_type = refine_label(adata, radius, key='stGRL')
        adata.obs['stGRL'] = new_type
